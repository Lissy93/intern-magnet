...
//Check the username is all cool
    ...
    $('form[name=signUp]').submit(function() {
        if($("#slctUserType").val()=="student"){
        element = "input#suLastname"
        if ($('input#suLastname').val() == '') {
            showFailMessage("Please enter your last name ", element);
            return false; }
        else if($('input#suLastname').val().length < 2){
            showFailMessage("Lastname must be atleast 3 characters long", element);
            return false; }
        else if($('input#suLastname').val().length > 20){
            showFailMessage("Last name must be under 20 characters long", element);
            return false; }
        else { return true; }
        } return true;
    });
...
$('form[name=signUp]').submit(function() {
    if($("#slctUserType").val()=="employer"){
        element = "input#suCompanyname"
        if ($('input#suCompanyname').val() == '') {
            showFailMessage("Please enter your company name ", element);
            return false; }
        else if($('input#suCompanyname').val().length < 2){
            showFailMessage("Compnany name must be at least 3 characters long", element);
            return false; }
        else if($('input#suCompanyname').val().length > 20){
            showFailMessage("Company name must be under 20 characters long", element);
            return false; }
        else { return true; }
    } return true;
});
...
/*
* show or hide appropriate parts of the form
 */

//Change form depending on employer or intren
$("#slctUserType").change(function () {
    if($(this).val()=="employer"){
        $("#part-student").slideUp('fast');
        $("#part-employer").slideDown('fast');
    }

    else if($(this).val()=="student"){
        $("#part-employer").slideUp('fast');
        $("#part-student").slideDown('fast');
    }
