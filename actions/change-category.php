...
// Set variables from POST parameters
    ...
    if($settings->changeCategory($category)){$success = true; } else{ $success = false; }


// Prepare message
    if($success){
        $message = $_SESSION['information-message']['message'] =  "Category Updated Successfully";
        $_SESSION['information-message']['type']    =  "success";
    }
    else{
        $message = $_SESSION['information-message']['message'] = "Unable to update category: ".$settings->getMessage();
        $_SESSION['information-message']['type']    =  "error"; }
